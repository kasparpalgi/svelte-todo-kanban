<!-- @file src/lib/components/notifications/UnifiedNotifications.svelte -->
<script lang="ts">
	import { Bell, Check, X, Trash2, Clock } from 'lucide-svelte';
...
<DropdownMenu>
	<Button variant="ghost" size="icon" class="relative h-9 w-9" slot="trigger">
		<Bell class="h-5 w-5" />
		{#if totalCount > 0}
			<span
