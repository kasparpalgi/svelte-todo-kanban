...
describe('Tracker Stats Calculation Logic', () => {
	...
	describe('Keyword Matching - Example 1 from spec', () => {
		it('should correctly allocate time as per specification example', () => {
			 ...
			 * project 1 - 10min
			 * Total: 70 min allocated = project 1 (40m) + work (20m) + entertainment (20m) - entertainment excluded
			 * Result: project 1 = 40m, work = 20m (unmatched if category unknown)
			 */
			...
			const sessions = [
				...
				{ title: 'unknown break', seconds: 3600 }, // 60 min - exceeds threshold
			...
			expect(matches[7].match).toBeNull(); // No match
			...
			// With 60-minute threshold, session exceeding it is excluded from totals
			const total_counted_seconds = matches
				.filter((_, i) => i !== 7) // Exclude the 60-min unmatched session
				.reduce((sum, m) => sum + m.seconds, 0);

			const total_all_seconds = matches.reduce((sum, m) => sum + m.seconds, 0);

			expect(project1_seconds).toBe(2400); // 4 * 600 = 40 minutes
			expect(work_seconds).toBe(1200); // 2 * 600 = 20 minutes
			expect(entertainment_seconds).toBe(1200); // 2 * 600 = 20 minutes

			const matched_percentage = (total_counted_seconds / total_all_seconds) * 100;
			expect(matched_percentage).toBeCloseTo(87.5, 1); // (7*600+3600) / 8*600+3600) = 75%
		});
	});
	...
	describe('Percentage Calculations', () => {
		it('should correctly calculate matched percentage', () => {
			// Only MATCHED sessions count toward matched percentage
			const matched_seconds = 3600; // 60 minutes with keywords
			const unmatched_seconds = 900; // 15 minutes unmatched (below threshold)
			const ignored_seconds = 3600; // 60 minutes unmatched (above threshold - ignored)

			const total_seconds = matched_seconds + unmatched_seconds + ignored_seconds;

			// Matched % = (sessions with keywords) / total
			const matched_percentage = (matched_seconds / total_seconds) * 100;

			expect(matched_percentage).toBeCloseTo(42.86, 1);

			// IMPORTANT: threshold doesn't affect matched %
			// Only sessions WITH keywords count as "matched"
			// Unmatched sessions (matched=false) are separate from ignored sessions
		});
		...
		it('should show that threshold only affects unmatched sessions', () => {
			...
			expect(unmatched_120).toBe(8400); // Sessions A + B + C = 600 + 1800 + 5400

			// But project time stays THE SAME
			expect(project_time).toBe(3600);

			// Because threshold only filters unmatched sessions
			// It doesn't affect matched sessions at all
		});
	});
